...
public class ProductPurchaseFromWebsite {
	...
	public void verifyUserLogin(WebElement emailInputField, WebElement passwordInputField, WebElement loginBtn,
		...
		sf.assertEquals(titleOfPage, "My Account", "User is not signed in");
	...
	public void selectPhonesTabFromNavBar(WebElement phonesPDAField) {
		...
		String titleOfPage = wd.getTitle();
		sf.assertEquals(titleOfPage, "Phones & PDAs", "Page is not valid");
	}

	public void selecedtPhone(WebElement selectedPhoneField) {
		selectedPhoneField.click();
		String titleOfPage = wd.getTitle();
		sf.assertEquals(titleOfPage, "Palm Treo Pro", "Page is not valid");
	...
	public void verifyItemInCartAndCheckout(WebElement itemInCartBtn) {
		...
		sf.assertEquals(text, "1 item(s) - $337.99", "No match for item");
	...
	public void verifyBillingDetails() {
		...
		sf.assertTrue(wd.findElement(By.cssSelector("input[name='shipping_method']")).isSelected(),
				"Element not selected");
		wd.findElement(By.cssSelector("input#button-shipping-method")).click();

		sf.assertTrue(wd.findElement(By.cssSelector("input[name='payment_method']")).isSelected(),
				"Element not selected");
		sleep();
		...
		sf.assertEquals(productText, "Palm Treo Pro");
		...
		sf.assertEquals(noOfItemText, "1");

		wd.findElement(By.cssSelector("input#button-confirm")).click();
		sleep();
		String titleOfPage = wd.getTitle();
		sf.assertEquals(titleOfPage, "Your order has been placed!", "order not placed");
	...
	public String userRegistration() {
		...
		String titleOfPage = wd.getTitle();
		sf.assertEquals(titleOfPage, "Your Account Has Been Created!", "Registration failed");
		return email;
	}
